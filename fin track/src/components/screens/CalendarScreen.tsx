    ...
    <div className="space-y-6">
      <div className="rounded-3xl bg-slate-800 p-8 text-white shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-slate-700 hover:bg-slate-600 transition-all duration-200 text-white font-medium shadow-lg"
          >
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-2xl bg-emerald-50 border border-emerald-200 p-4 shadow-sm">
          <p className="text-emerald-700 text-sm mb-1 font-medium">Income</p>
          <p className="text-emerald-900 text-lg font-bold">${monthlyTotals.income.toLocaleString()}</p>
        </div>
        <div className="rounded-2xl bg-rose-50 border border-rose-200 p-4 shadow-sm">
          <p className="text-rose-700 text-sm mb-1 font-medium">Expenses</p>
          <p className="text-rose-900 text-lg font-bold">${monthlyTotals.expenses.toLocaleString()}</p>
        </div>
        <div className="rounded-2xl bg-slate-100 border border-slate-200 p-4 shadow-sm">
          <p className="text-slate-700 text-sm mb-1 font-medium">Net</p>
          <p className="text-slate-900 text-lg font-bold">${monthlyTotals.balance.toLocaleString()}</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl bg-white p-6 border border-slate-200 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-center">
            <h3 className="text-slate-900 text-xl font-bold">{monthNames[month]} {year}</h3>
            <button
              onClick={goToToday}
        ...
        <div className="grid grid-cols-7 gap-2 mb-2">
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
            <div key={day} className="text-center text-slate-600 text-sm py-2 font-medium">
              {day}
            </div>
          ))}
        </div>
