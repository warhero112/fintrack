...
export function AIAdvisorScreen() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-200px)]">
      <div className="rounded-2xl bg-slate-800 p-8 text-white border border-slate-700 mb-6">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-slate-700 flex items-center justify-center">
            <Bot className="w-8 h-8 text-white" />
          </div>
          <div>
            <h2 className="text-white mb-1">AI Financial Advisor</h2>
            <p className="text-slate-300">Powered by advanced AI</p>
          </div>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto mb-6 space-y-4 rounded-2xl bg-white p-6 border border-slate-200">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          {quickQuestions.map((question, index) => (
            <button
              ...
              className="px-4 py-2 rounded-xl bg-white border border-slate-200 text-slate-900 text-sm hover:shadow-md transition-all text-left"
            >
              {question}
            </button>
          ))}
      ...
      <div className="rounded-2xl bg-white p-4 border border-slate-200">
        <div className="flex gap-3">
          ...
          <button
            onClick={() => handleSend()}
            disabled={!input.trim()}
            className="px-6 py-4 rounded-2xl bg-slate-800 text-white hover:bg-slate-700 transition-all disabled:opacity-50 disabled:hover:bg-slate-800"
          >
